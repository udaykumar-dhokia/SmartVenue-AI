...
const MENU = [
  ...
  { name: "Gujarat", price: 80, category: "Snack" },
...
export function PreOrder() {
  ...
  const removeItem = (name: string) => {
    setCart((prev) =>
      prev
        .map((i) =>
          i.name === name ? { ...i, qty: Math.max(0, i.qty - 1) } : i,
        )
        .filter((i) => i.qty > 0),
    );
  };
  ...
  const nearestOutlet = CONCESSION_OUTLETS.reduce(
    (best, o) => (o.waitTime < best.waitTime ? o : best),
    CONCESSION_OUTLETS[0],
  );
    ...
    >
      <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 flex items-center gap-3">
        ...
        <div>
          <p className="text-xs font-medium text-emerald-300">
            Nearest outlet: {nearestOutlet.name}
          </p>
          <p className="text-[10px] text-zinc-500">
            Wait time: {nearestOutlet.waitTime} min | {nearestOutlet.zone}
          </p>
        </div>
      </div>
          ...
          <div className="space-y-1.5">
            {MENU.filter((m) => m.category === cat).map((item) => {
              ...
              return (
                <div
                  key={item.name}
                  className="flex items-center justify-between rounded-lg border border-zinc-800/40 bg-zinc-900/30 px-3 py-2.5"
                >
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    {inCart ? (
                      ...
                      <button
                        onClick={() => addItem(item.name, item.price)}
                        className="px-3 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-[10px] font-semibold text-white transition-colors"
                      >
                        Add
                      </button>
                    )}
      ...
      {totalItems > 0 && (
        <motion.div
          ...
          className="sticky bottom-0 rounded-xl bg-indigo-600 p-4 flex items-center justify-between shadow-lg shadow-indigo-500/20"
        >
          <div>
